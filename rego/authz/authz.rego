...
# extract_namespaces: Extract namespaces from the nested extras.selectors structure
...
tenant_signal_access(rule, request_input) if {
	element_match(object.get(rule, "tenants", []), request_input.tenant)
...
# For metadata read operations
permission_granted(rule, request_input) if {
	request_input.permission == "read"
	access_verb_matches(rule, request_input.permission)
	metadata_only := object.get(request_input.extras, "metadataOnly", false)
...
permission_granted(rule, request_input) if {
	request_input.permission == "read"
	access_verb_matches(rule, request_input.permission)
	read_scope_validation(rule, request_input)
}

read_scope_validation(rule, request_input) if {
	scope := rule_scope(rule)
	scope == "application"
	requested_namespaces := extract_namespaces(request_input)
	every namespace in requested_namespaces {
		element_match(object.get(rule, "namespaces", []), namespace)
	}
}
...
read_scope_validation(rule, request_input) if {
	...
	requested_namespaces := extract_namespaces(request_input)
	every namespace in requested_namespaces {
		# Infrastructure scope allows access to infrastructure namespaces only
		infrastructure_namespace_match(namespace)
	}
}
...
# element_match: True if the requested tenant is allowed by the rule.
